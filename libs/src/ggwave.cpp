...
int ggwave_encode(
    ...
    if (ggWave == nullptr) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Invalid GGWave instance %d\n", id);
        return -1;
    }
    ...
    if (ggWave->init(payloadSize, (const char *) payloadBuffer, protocolId, volume) == false) {
        ggprintf("Failed to initialize Tx transmission for GGWave instance %d\n", id);
         __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Failed to initialize Tx transmission for GGWave instance %d\n", id);
         __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Payload Size %d\n", payloadSize);
         __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Protocol Id %d\n", protocolId);
         __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Volume %d\n", volume);
          __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Buffer %d\n", payloadBuffer);
        return -1;
    }
    ...
    if (nBytes == 0) {
        ggprintf("Failed to encode data - GGWave instance %d\n", id);
         __android_log_print(ANDROID_LOG_DEBUG, "GGWave", "Failed to encode data - GGWave instance %d\n", id);
        return -1;
    }
